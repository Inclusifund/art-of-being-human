...
(() => {
  ...
  const HIDDEN_KEY  = 'inclusifund-calm-kit-hidden';
  ...
  const mount = () => {
    ...
    const html = `
      <aside id="calm-kit" aria-label="Accessibility settings">
        <button class="ck-tab" type="button" aria-expanded="false" aria-controls="ck-panel">
          <span class="ck-tab-icon" aria-hidden="true"></span>
          <span class="ck-tab-label">Calm kit</span>
        </button>
        <button class="ck-dismiss" type="button" aria-label="Hide calm kit">×</button>
    ...
    const dismissBtn = panel.querySelector('.ck-dismiss');

    // Backdrop — mounted as sibling, visible on mobile when panel is open
    const backdrop = document.createElement('div');
    backdrop.className = 'ck-backdrop';
    backdrop.setAttribute('aria-hidden', 'true');
    document.body.appendChild(backdrop);

    // Restore pill — sibling element, mounted alongside the kit
    const restore = document.createElement('button');
    restore.className = 'ck-restore';
    restore.type = 'button';
    restore.setAttribute('aria-label', 'Show accessibility settings');
    restore.innerHTML = '✦';
    document.body.appendChild(restore);

    const setHidden = (hidden) => {
      document.body.classList.toggle('ck-hidden', hidden);
      try { localStorage.setItem(HIDDEN_KEY, hidden ? '1' : '0'); } catch (_) {}
    };

    // Apply prior dismiss preference
    try {
      if (localStorage.getItem(HIDDEN_KEY) === '1') document.body.classList.add('ck-hidden');
    } catch (_) {}
